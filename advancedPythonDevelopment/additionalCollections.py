...
coworker_alma_maters = defaultdict(
    list)  # remember list is a function, returns [], here by default if key doesn't exist
...
print(coworker_alma_maters['Tolik'])  # []

for key, value in coworker_alma_maters.items():
    print(f"BOOYA {key}: {value}")
...
coworker_companies = defaultdict(lambda: my_company)  # lambda, because argument has to be a function
...
inventory = defaultdict(int)  # When int is called without any arguments, it returns 0
...
# user_input = input("Enter movie values: ")
# movie = Movie(*user_input.strip().split(","))
# print(f"{movie.title}, {movie.director}, {movie.year}, {movie.budget}")
...
for char in value:
    dict_count[char] += 1
most_common_character = max(dict_count, key=lambda key: dict_count[key])
# The function returns the values of dictionaries. Based on the values (rather than the dictionary's keys),
# the key having the maximum value is returned.
print(most_common_character)
print(dict_count[most_common_character])
